...
final class ProviderFactory
{
    public function routing(): RoutingProviderInterface
    {
        if (get_option('pov_routing_provider') === 'heigit' && (string) get_option('pov_heigit_api_key') !== '') {
            $primary = new OpenRouteServiceRoutingProvider(
                'https://api.heigit.org/openrouteservice/',
                (string) get_option('pov_heigit_api_key')
            );
            $fallbackUrl = trim((string) get_option('pov_routing_base_url', ''));
            if ($fallbackUrl !== '') {
                return new FallbackRoutingProvider($primary, new OsrmRoutingProvider($fallbackUrl));
            }
            return $primary;
        }
    ...
    public function geocoding(): GeocodingProviderInterface
    {
        ...
        if (get_option('pov_geocoding_provider') === 'heigit' && (string) get_option('pov_heigit_api_key') !== '') {
            $primary = new PeliasGeocodingProvider(
                'https://api.heigit.org/pelias/v1/',
                (string) get_option('pov_heigit_api_key')
            );
            $fallbackUrl = trim((string) get_option('pov_geocoding_base_url', ''));
            if ($fallbackUrl !== '') {
                return new FallbackGeocodingProvider($primary, new NominatimGeocodingProvider($fallbackUrl));
            }
            return $primary;
        }
